...
public class MarController {
    ...
    public ResponseEntity<?> getLatestState(@RequestParam(required = false) String email) {
        try {
            List<Block> chain = blockchainService.getMarBlocks();
    ...
    public ResponseEntity<?> saveState(@RequestBody Map<String, Object> statePayload) {
        try {
            String teacherEmail = statePayload.containsKey("email") ? (String) statePayload.get("email") : "mar@system";
            String json = objectMapper.writeValueAsString(statePayload);

            boolean added = blockchainService.addBlock(
                    "MAR_RECORD", teacherEmail, "MAR",
                    "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", json
            );

            if (added) {
                return ResponseEntity.ok(Map.of("status", "success", "message", "MAR saved to blockchain"));
            } else {
